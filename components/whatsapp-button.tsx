...
const WA_MESSAGE = "¡Hola! Quiero consultar sobre los sorteos de Lotería de Animales."

/** Limpia el número: quita espacios/guiones/paréntesis, convierte 0xxx → 595xxx */
function buildWhatsAppUrl(raw: string): string {
  const digits = raw.replace(/\D/g, "").replace(/^0/, "595")
  return `https://wa.me/${digits}?text=${encodeURIComponent(WA_MESSAGE)}`
}

export async function WhatsAppButton() {
  let href = `https://wa.me/595981000000?text=${encodeURIComponent(WA_MESSAGE)}` // fallback
